...
## Versions: 
The current plugin release version is 1.0.0.0; this is available in the 'releases' section. The most recently compiled .jar file that runs (which may be the same as the most recent release but at the moment is version 1.0.0.1) can be found in the 'Jar' folder. This plugin is designed to be run with Minecraft Spigot running versions between 1.7.x and 1.12.x. Most versions of Vault and PlayerPoints for these Spigot versions should work. Bugs found when running on the aforementioned Minecraft versions will be fixed. The plugin may still work with other versions but bugs for these versions will not necessarily be fixed. This plugin should work with Minecraft version 1.13.1 but this is as yet not fully tested. 
